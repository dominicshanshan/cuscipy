...
def test_webuse():
    ...
    res2 = cp.array([list(row) for row in macrodata_result])
    ...
    try:
        ...
        pytest.skip('Failed with HTTPError or URLError, these are random')
    assert_array_equal(res1.values, res2)


def test_webuse_pandas():
    # test copied and adjusted from iolib/tests/test_foreign
    from statsmodels.compat.c_pandas import assert_frame_equal
    ...
    assert_frame_equal(res1.to_pandas(), dta.astype(float).to_pandas())
