...
import cupy as cp
import numpy as np
from cudf import read_csv, DataFrame, Series, Index
...
def webuse(data, baseurl='https://www.stata-press.com/data/r11/', as_df=True):
    ...
    return cudf.from_pandas(read_stata(url))
...
def _get_dataset_meta(dataname, package, cache):
    ...
    idx = cp.logical_and(index.Item == dataname, index.Package == package)
...
def as_cupy_dataset(ds, as_pandas=True, retain_index=False):
    """Convert a dataframe dataset to a Cupy dataset"""
    if as_pandas:
        return ds
    ds.data = ds.data.to_pandas().to_records(index=retain_index)
    for d in dir(ds):
        if d.startswith('_'):
            continue
        attr = getattr(ds, d)
        if isinstance(attr, Series):
            setattr(ds, d, cp.asarray(attr))
        if isinstance(attr, DataFrame):
            dtypes = attr.dtypes.to_dict()
            cols = []
            for col in dtypes:
                if dtypes[col] != np.dtype('O'):
                    cols.append(col)
            attr = attr[cols]
            try:
                attr = attr.fillna(cp.nan)
            except:
                pass
            setattr(ds, d, attr.values)
