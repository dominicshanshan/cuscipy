...
def gen_data(dim, use_cudf):
    if dim == 1:
        ...
        out = cp.empty((20, 10))
        if use_cudf:
            out = cudf.DataFrame(out)
    else:
        out = cp.empty(tuple(cp.arange(5, 5 + dim).get()))
...
class TestArrayLike(object):
    ...
    def test_right_squeeze_and_pad(self):
        ...
        data = cp.empty((2, 1, 1, 1))
        a = array_like(data, "a", ndim=3)
        assert a.shape == (2, 1, 1)

        data = cp.empty((2, 1, 1, 2, 1, 1))
    ...
    def test_dtype(self):
        x = cp.arange(10)
        a = array_like(x, "a", dtype=cp.float32)
        assert a.dtype == cp.float32

        a = array_like(x, "a", dtype=cp.uint8)
        assert a.dtype == cp.uint8
...
def test_right_squeeze():
    ...
    x = cp.empty((10, 10, 1, 1, 1, 1, 1))
    y = _right_squeeze(x)
    assert y.shape == (10, 10)

    x = cp.empty((10, 1, 10, 1, 1, 1, 1, 1))
    y = _right_squeeze(x)
    assert y.shape == (10, 1, 10)
...
def test_wrap_pandas(use_pandas):
    ...
    expected_type = cudf.Series if use_pandas else cp.ndarray
    ...
    expected_type = cudf.DataFrame if use_pandas else cp.ndarray
...
@pytest.fixture(params=(1.0, 1.1, cp.float32(1.2), cp.array([1.2]), 1.2 + 0j))
def floating(request):
    return request.param


@pytest.fixture(params=(cp.empty(2), 1.2 + 1j, True, "3.2", None))
...
def test_float_like(floating):
    ...
    if isinstance(floating, (int, cp.integer, float, cp.inexact)):
...
@pytest.fixture(params=(1.0, 2, cp.float32(3.0), cp.array([4.0])))
def integer(request):
    return request.param
...
def test_int_like(integer):
    ...
    if isinstance(integer, (int, cp.integer)):
...
def test_not_bool_like():
    with pytest.raises(TypeError):
        bool_like(cp.array([True, True]), boolean)
