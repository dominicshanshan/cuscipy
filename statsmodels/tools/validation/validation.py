...
def array_like(
    ...
    dtype : {None, cupy.dtype, str}
        Required dtype. Default is double. If None, does not change the dtype
        of obj (if present) or uses cupy to automatically detect the dtype
    ndim : {int, None}
    ...
    >>> x = cp.arange(4*10*4).reshape((4, 10, 4))
    ...
    if isinstance(obj, cudf.DataFrame):
        ...
        arr = cp.asarray(obj, dtype=dtype, order=order)
    if maxdim is not None:
        ...
        if arr.ndim > ndim:
            ...
            arr = cp.reshape(arr, arr.shape + (1,) * (ndim - arr.ndim))
        if arr.ndim != ndim:
    ...
    if contiguous:
        arr = cp.ascontiguousarray(arr, dtype=dtype)
    return arr
...
class PandasWrapper(object):
    ...
    def __init__(self, cudf_obj):
        self._pandas_obj = cudf_obj
        self._is_cudf = isinstance(cudf_obj, (cudf.Series, cudf.DataFrame))
    ...
    def wrap(self, obj, columns=None, append=None, trim_start=0, trim_end=0):
        ...
        if obj.ndim == 1:
            ...
            return cudf.Series(obj, name=name, index=index)
        elif obj.ndim == 2:
            if columns is None:
            ...
            if append is not None:
                ...
                columns = new
            return cudf.DataFrame(obj, columns=columns, index=index)
        else:
...
def int_like(
    ...
    optional : bool
        Flag indicating whether None is allowed
    strict : bool
        If True, then only allow int or cp.integer that are not bool. If False,
    ...
    if optional and value is None:
        return None
    # is_bool_timedelta = isinstance(value, (bool, cp.timedelta64))
    is_bool_timedelta = isinstance(value, (bool,))

    if hasattr(value, "squeeze") and callable(value.squeeze):
        value = value.squeeze()

    if isinstance(value, (int, cp.integer)) and not is_bool_timedelta:
        return int(value)
    ...
    extra_text = " or None" if optional else ""
    raise TypeError(
        "{0} must be integer_like (int or cp.integer, but not bool"
...
def float_like(value, name, optional=False, strict=False):
    ...
    optional : bool
        Flag indicating whether None is allowed
    strict : bool
        If True, then only allow int, cp.integer, float or cp.inexact that are
    ...
    is_complex = isinstance(value, (complex, cp.complexfloating))
    if hasattr(value, "squeeze") and callable(value.squeeze):
        value = value.squeeze()

    if isinstance(value, (int, cp.integer, float, cp.inexact)) and not (
    ...
    extra_text = " or None" if optional else ""
    raise TypeError(
        "{0} must be float_like (float or cp.inexact)"
        "{1}".format(name, extra_text)
    )
