...
def mse(x1, x2, axis=0):
    ...
    This uses ``cupy.asanyarray`` to convert the input. Whether this is the
    desired result or not depends on the array subclass, for example
    cupy matrices will silently produce an incorrect result.
    """
    x1 = cp.asanyarray(x1)
    x2 = cp.asanyarray(x2)
    return cp.mean((x1 - x2) ** 2, axis=axis)
...
def rmse(x1, x2, axis=0):
    ...
    x1 = cp.asanyarray(x1)
    x2 = cp.asanyarray(x2)
    return cp.sqrt(mse(x1, x2, axis=axis))


def rmspe(y, y_hat, axis=0, zeros=cp.nan):
    """
    ...
    """
    y_hat = cp.asarray(y_hat)
    y = cp.asarray(y)
    error = y - y_hat
    loc = y != 0
    # loc = loc.ravel()
    percentage_error = cp.full_like(error, zeros)
    # percentage_error.flat[loc] = error.flat[loc] / y.flat[loc]
    percentage_error[loc] = error[loc] / y[loc]
    mspe = cp.nanmean(percentage_error ** 2, axis=axis) * 100
    return cp.sqrt(mspe)
...
def maxabs(x1, x2, axis=0):
    ...
    x1 = cp.asanyarray(x1)
    x2 = cp.asanyarray(x2)
    return cp.max(cp.abs(x1 - x2), axis=axis)
...
def meanabs(x1, x2, axis=0):
    ...
    x1 = cp.asanyarray(x1)
    x2 = cp.asanyarray(x2)
    return cp.mean(cp.abs(x1 - x2), axis=axis)
...
def medianabs(x1, x2, axis=0):
    ...
    x1 = cp.asanyarray(x1)
    x2 = cp.asanyarray(x2)
    return cp.median(cp.abs(x1 - x2), axis=axis)
...
def bias(x1, x2, axis=0):
    ...
    x1 = cp.asanyarray(x1)
    x2 = cp.asanyarray(x2)
    return cp.mean(x1 - x2, axis=axis)
...
def medianbias(x1, x2, axis=0):
    ...
    x1 = cp.asanyarray(x1)
    x2 = cp.asanyarray(x2)
    return cp.median(x1 - x2, axis=axis)
...
def vare(x1, x2, ddof=0, axis=0):
    ...
    x1 = cp.asanyarray(x1)
    x2 = cp.asanyarray(x2)
    return cp.var(x1 - x2, ddof=ddof, axis=axis)
...
def stde(x1, x2, ddof=0, axis=0):
    ...
    x1 = cp.asanyarray(x1)
    x2 = cp.asanyarray(x2)
    return cp.std(x1 - x2, ddof=ddof, axis=axis)
...
def iqr(x1, x2, axis=0):
    ...
    if axis is None:
        ...
        x2 = x2.ravel()
        axis = 0
    xdiff = cp.sort(x1 - x2, axis=axis)
    nobs = x1.shape[axis]
    idx = cp.around((nobs - 1) * cp.array([0.25, 0.75])).astype(int)
    sl = [slice(None)] * xdiff.ndim
    sl[axis] = idx
    iqr = cp.diff(xdiff[tuple(sl)], axis=axis)
    iqr = cp.squeeze(iqr)  # drop reduced dimension
    return iqr
...
def bic(llf, nobs, df_modelwc):
    ...
    """
    return -2.0 * llf + cp.log(nobs) * df_modelwc
...
def hqic(llf, nobs, df_modelwc):
    ...
    """
    return -2.0 * llf + 2 * cp.log(cp.log(nobs)) * df_modelwc
...
# def fpe_sigma():
#     ((nobs + self.df_model) / self.df_resid) ** neqs * cp.exp(ld)
