...
from statsmodels.compat.c_pandas import Appender, Substitution
...
def _get_epsilon(x, s, epsilon, n):
    if epsilon is None:
        h = EPS**(1. / s) * cp.maximum(cp.abs(x), 0.1)
    else:
        if cp.isscalar(epsilon):
            h = cp.empty(n)
            h.fill(epsilon)
        else:  # pragma : no cover
            h = cp.asarray(epsilon)
            if h.shape != x.shape:
...
def approx_fprime(x, f, epsilon=None, args=(), kwargs={}, centered=False):
    ...
    f0 = f(*((x,)+args), **kwargs)
    dim = cp.atleast_1d(f0).shape  # it could be a scalar
    grad = cp.zeros((n,) + dim, cp.promote_types(float, x.dtype))
    ei = cp.zeros((n,), float)
    if not centered:
...
def approx_hess1(x, f, epsilon=None, args=(), kwargs={}, return_grad=False):
    ...
    hess = cp.outer(h, h)  # this is now epsilon**2
