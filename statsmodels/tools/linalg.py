...
def logdet_symm(m, check_symm=False):
    ...
    if check_symm:
        if not cp.all(m == m.T):  # would be nice to short-circuit check
            raise ValueError("m is not symmetric.")
    c, _ = linalg.cho_factor(m, lower=True)
    return 2*cp.sum(cp.log(c.diagonal()))
...
def stationary_solve(r, b):
    ...
    for j in range(1, len(b)):
        rf = r[0:j][::-1]
        a = (b[j, :] - cp.dot(rf, x)) / (1 - cp.dot(rf, db[::-1]))
        z = x - cp.outer(db[::-1], a)
        x = cp.concatenate((z, a[None, :]), axis=0)

        if j == len(b) - 1:
            break

        rn = r[j]
        a = (rn - cp.dot(rf, db)) / (1 - cp.dot(rf, db[::-1]))
        z = db - a*db[::-1]
        db = cp.concatenate((z, cp.r_[a]))
...
def transf_constraints(constraints):
    ...
    q, _ = linalg.qr(cp.transpose(constraints))
    transf = q[:, m:]
    return transf
...
def matrix_sqrt(mat, inverse=False, full=False, nullspace=False,
    ...
    sqrt_s = cp.sqrt(s[mask])
    if inverse:
        sqrt_s = 1 / cp.sqrt(s[mask])

    if full:
        b = cp.dot(u[:, mask], cp.dot(cp.diag(sqrt_s), v[mask]))
    else:
        b = cp.dot(cp.diag(sqrt_s), v[mask])
    return b
