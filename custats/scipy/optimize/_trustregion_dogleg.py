...
class DoglegSubproblem(BaseQuadraticSubproblem):
    ...
    def cauchy_point(self):
        ...
        if self._cauchy_point is None:
            g = self.jac
            Bg = self.hessp(g)
            self._cauchy_point = -(cp.dot(g, g) / cp.dot(g, Bg)) * g
    ...
    def newton_point(self):
        ...
        if self._newton_point is None:
            g = self.jac
            B = self.hess
            cho_info = cp.linalg.cho_factor(B)
            self._newton_point = -cp.linalg.cho_solve(cho_info, g)
    ...
    def solve(self, trust_radius):
        ...
        if cp.linalg.norm(p_best) < trust_radius:
            hits_boundary = False
