...
class ScalarFunction(object):
    ...
    finite_diff_bounds : tuple of array_like
        ...
        (-cp.inf, cp.inf). Each bound must match the size of `x0` or be a
    ...
    def __init__(self, fun, x0, args, grad, hess, finite_diff_rel_step,
        ...
        self.x = cp.atleast_1d(x0).astype(float)
        self.n = self.x.size
        self.nfev = 0
        self.ngev = 0
        self.nhev = 0
        ...
        if callable(grad):
            def grad_wrapped(x):
                self.ngev += 1
                return cp.atleast_1d(grad(cp.copy(x), *args))
        ...
        if callable(hess):
            ...
            if sps.issparse(self.H):
                ...
                def hess_wrapped(x):
                    self.nhev += 1
                    return cp.atleast_2d(cp.asarray(hess(x, *args)))
                if self.H is None:
                    self.H = cp.atleast_2d(cp.empty((0)))
                else:
                    self.H = cp.atleast_2d(cp.asarray(self.H))
        ...
        if isinstance(hess, HessianUpdateStrategy):
            def update_x(x):
                ...
                self.x = cp.atleast_1d(x).astype(float)
                self.f_updated = False
                self.g_updated = False
                self.H_updated = False
                self._update_hess()
    ...
    def fun(self, x):
        if not cp.array_equal(x, self.x):
...
class VectorFunction(object):
    ...
    def __init__(self, fun, x0, jac, hess,
        ...
        if callable(jac):
            ...
            if (sparse_jacobian or
                ...
                def jac_wrapped(x):
                    self.njev += 1
                    return cp.atleast_2d(jac(x))
                self.J = cp.atleast_2d(self.J)
                self.sparse_jacobian = False
        ...
        if callable(hess):
            ...
            if sps.issparse(self.H):
                ...
                def hess_wrapped(x, v):
                    self.nhev += 1
                    return cp.atleast_2d(cp.asarray(hess(x, v)))
                self.H = cp.atleast_2d(cp.asarray(self.H))
    ...
    def _update_v(self, v):
        if not cp.array_equal(v, self.v):
            self.v = v
            self.H_updated = False
...
class LinearVectorFunction(object):
    ...
    def __init__(self, A, x0, sparse_jacobian):
        if sparse_jacobian or sparse_jacobian is None and sps.issparse(A):
            ...
            # cp.asarray makes sure A is ndarray and not matrix
            self.J = cp.atleast_2d(cp.asarray(A))
            self.sparse_jacobian = False
        ...
        self.v = cp.zeros(self.m, dtype=float)
