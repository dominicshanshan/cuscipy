...
from ._differentiable_functions import (VectorFunction, LinearVectorFunction,
...
def _arr_to_scalar(x):
    ...
    return x.item() if isinstance(x, cp.ndarray) else x
...
class NonlinearConstraint(object):
    ...
    lb, ub : array_like
        ...
        for all components of the constraint. Use ``cp.inf`` with an
    ...
    jac : {callable,  '2-point', '3-point', 'cs'}, optional
        ...
        Default is '2-point'.
    hess : {callable, '2-point', '3-point', 'cs', HessianUpdateStrategy, None},
        optional Method for computing the Hessian matrix. The keywords
        ...
        ``hess(x, v) -> {LinearOperator, sparse matrix, array_like},
        shape (n, n)``.  Here ``v`` is ndarray with shape (m,) containing
    ...
    >>> con = lambda x: x[0] - cp.sin(x[1])
    >>> nlc = NonlinearConstraint(con, -cp.inf, 1.9)
...
class Bounds(object):
    ...
    lb, ub : array_like, optional
        ...
        to fix a variable. Use ``cp.inf`` with an appropriate sign to disable
...
class PreparedConstraint(object):
    ...
    def __init__(self, constraint, x0, sparse_jacobian=None,
                 finite_diff_bounds=(-cp.inf, cp.inf)):
        ...
        m = fun.m
        lb = cp.asarray(constraint.lb, dtype=float)
        ub = cp.asarray(constraint.ub, dtype=float)
        if lb.ndim == 0:
            lb = cp.resize(lb, m)
        if ub.ndim == 0:
            ub = cp.resize(ub, m)

        keep_feasible = cp.asarray(constraint.keep_feasible, dtype=bool)
        if keep_feasible.ndim == 0:
            keep_feasible = cp.resize(keep_feasible, m)
        if keep_feasible.shape != (m,):
            raise ValueError("`keep_feasible` has a wrong shape.")

        mask = keep_feasible & (lb != ub)
        f0 = fun.f
        if cp.any(cp.array(f0[mask] < lb[mask])) or cp.any(cp.array(f0[mask] > ub[mask])):
    ...
    def violation(self, x):
        ...
        with suppress_warnings() as sup:
            ...
            ev = self.fun.fun(cp.asarray(x))

        excess_lb = cp.maximum(self.bounds[0] - ev, 0)
        excess_ub = cp.maximum(ev - self.bounds[1], 0)
...
def new_bounds_to_old(lb, ub, n):
    ...
    variable.
    If any of the entries in lb/ub are -cp.inf/cp.inf they are replaced by
    None.
    """
    lb = cp.asarray(lb)
    ub = cp.asarray(ub)
    ...
    lb = [float(x) if x > -cp.inf else None for x in lb]
    ub = [float(x) if x < cp.inf else None for x in ub]
...
def old_bound_to_new(bounds):
    ...
    lb = cp.array([float(_arr_to_scalar(x)) if x is not None else -cp.inf
                   for x in lb])
    ub = cp.array([float(_arr_to_scalar(x)) if x is not None else cp.inf
                   for x in ub])
...
def strict_bounds(lb, ub, keep_feasible, n_vars):
    ...
    strict_lb = cp.resize(lb, n_vars).astype(float)
    strict_ub = cp.resize(ub, n_vars).astype(float)
    keep_feasible = cp.resize(keep_feasible, n_vars)
    strict_lb[~keep_feasible] = -cp.inf
    strict_ub[~keep_feasible] = cp.inf
...
def new_constraint_to_old(con, x0):
    ...
    i_eq = lb == ub
    i_bound_below = cp.logical_xor(lb != -cp.inf, i_eq)
    i_bound_above = cp.logical_xor(ub != cp.inf, i_eq)
    i_unbounded = cp.logical_and(lb == -cp.inf, ub == cp.inf)

    if cp.any(i_unbounded):
    ...
    cineq = []
    n_bound_below = cp.sum(i_bound_below).item()
    n_bound_above = cp.sum(i_bound_above).item()
    if n_bound_below + n_bound_above:
        def f_ineq(x):
            # import pudb
            # pudb.set_trace()
            y = cp.zeros(n_bound_below + n_bound_above)
            y_all = cp.array(fun(x)).flatten()
        ...
        if jac is not None:
            def j_ineq(x):
                dy = cp.zeros((n_bound_below + n_bound_above, len(x0)))
                dy_all = jac(x)
                if issparse(dy_all):
                    dy_all = dy_all.todense()
                dy_all = cp.atleast_2d(dy_all)
