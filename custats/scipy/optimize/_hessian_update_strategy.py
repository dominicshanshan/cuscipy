...
class FullHessianUpdateStrategy(HessianUpdateStrategy):
    ...
    """
    # Commenting out as these are not used for custats
    #_syr = get_blas_funcs2('syr', dtype='d')  # Symmetric rank 1 update
    #_syr2 = get_blas_funcs2('syr2', dtype='d')  # Symmetric rank 2 update
    # Symmetric matrix-vector product
    #_symv = get_blas_funcs2('symv', dtype='d')
    ...
    def initialize(self, n, approx_type):
        ...
        if self.approx_type == 'hess':
            self.B = cp.eye(n, dtype=float)
        else:
            self.H = cp.eye(n, dtype=float)

    def _auto_scale(self, delta_x, delta_grad):
        # Heuristic to scale matrix at first iteration.
        # Described in Nocedal and Wright "Numerical Optimization"
        # p.143 formula (6.20).
        s_norm2 = cp.dot(delta_x, delta_x)
        y_norm2 = cp.dot(delta_grad, delta_grad)
        ys = cp.abs(cp.dot(delta_grad, delta_x))
    ...
    def update(self, delta_x, delta_grad):
        ...
        delta_grad : ndarray
            ...
            ``delta_grad = grad(x2) - grad(x1)``.
        """
        if cp.all(delta_x == 0.0):
            return
        if cp.all(delta_grad == 0.0):
        ...
        if self.first_iteration:
            ...
            # Scale initial matrix with ``scale * cp.eye(n)``
            if self.approx_type == 'hess':
                self.B *= scale
            else:
                self.H *= scale
...
class BFGS(FullHessianUpdateStrategy):
    ...
    init_scale : {float, 'auto'}
        ...
        with ``init_scale*cp.eye(n)``, where ``n`` is the problem dimension.
...
#
    ...
    def _update_implementation(self, delta_x, delta_grad):
        ...
        if wMw <= 0.0:
            ...
            # Reinitialize matrix
            if self.approx_type == 'hess':
                self.B = scale * cp.eye(self.n, dtype=float)
            else:
                self.H = scale * cp.eye(self.n, dtype=float)
...
class SR1(FullHessianUpdateStrategy):
    ...
    #    Mw = self.dot(w)
    #    z_minus_Mw = z - Mw
    #    denominator = cp.dot(w, z_minus_Mw)
    #    # If the denominator is too small
    #    # we just skip the update.
    #    if cp.abs(denominator) <= self.min_denominator*norm(w)*norm(z_minus_Mw):
    #        return
    #    # Update matrix
    #    if self.approx_type == 'hess':
