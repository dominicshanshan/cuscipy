...
# from custats.core.testing import (assert_, assert_allclose)
from pytest import raises as assert_raises
import pytest
import cupy as cp
from numpy import errstate

from custats.scipy.optimize import (
    fmin_slsqp, minimize, Bounds, NonlinearConstraint)
...
class TestSLSQP:
    ...
    def fun(self, d, sign=1.0):
        ...
        x = d[0]
        y = d[1]
        # print('v', sign*(2*x*y + 2*x - x**2 - 2*y**2), 'd', d)
    ...
    def jac(self, d, sign=1.0):
        ...
        dfdx = sign*(-2*x + 2*y + 2)
        dfdy = sign*(2*x - 4*y)
        # print('jac', cp.array([dfdx, dfdy], float))
        return cp.array([dfdx, dfdy], float)
    ...
    def test_minimize_unbounded_approximated(self):
        ...
        for jac in jacs:
            ...
            assert_allclose(res.x, cupy.array([2, 1]))
    ...
    def test_minimize_bounded_approximated(self):
        ...
        for jac in jacs:
            with errstate(invalid='ignore'):
                print(self.fun([-1.0, 1.0]), 'jac', jac)
                res = minimize(self.fun, [-1.0, 1.0], args=(-1.0, ),
                               jac=jac,
    ...
    def test_unbounded_approximated(self):
        ...
        assert_array_almost_equal(x.get(), [2, 1])
    ...
    def test_equality_approximated(self):
        ...
        x, fx, its, imode, smode = res
        assert_(imode == 0, imode)
        assert_array_almost_equal(x.get(), [1, 1])
    ...
    def test_inequality_given(self):
        ...
        x, fx, its, imode, smode = res
        assert_(imode == 0, imode)
        assert_array_almost_equal(x.get(), [2, 1], decimal=3)
    ...
    def test_bound_equality_given2(self):
        ...
        x, fx, its, imode, smode = res
        assert_(imode == 0, imode)
        assert_array_almost_equal(x.get(), [0.8, 0.8], decimal=3)
        assert_(-0.8 <= x[0] <= 1)
        assert_(-1 <= x[1] <= 0.8)
    ...
    def test_scalar_constraints(self):
        ...
        x = fmin_slsqp(lambda z: z**2, [3.],
                       ...
                       iprint=0)
        assert_array_almost_equal(x.get(), [1.])
    ...
    def test_array_bounds(self):
        ...
        bounds = [(-cp.inf, cp.inf), (cp.array([2]), cp.array([3]))]
        x = fmin_slsqp(lambda z: cp.sum(z**2 - 1), [2.5, 2.5], bounds=bounds,
                       iprint=0)
        assert_array_almost_equal(x.get(), [0, 2])
    ...
    def test_obj_returns_scalar_in_list(self):
        ...
        fmin_slsqp(lambda x: cupy.array([0]), [1, 2, 3], iprint=0)
    ...
    def test_new_bounds_type(self):
        ...
        bounds = Bounds([1, 0], [cp.inf, cp.inf])
    ...
    def test_gh1758(self):
        ...
        cp.testing.assert_allclose(res.fun, 0.5443310539518)
        cp.testing.assert_allclose(res.x, [0.33333333, 0.2962963])
        print(res)
        print(c1['fun'](res.x))
        print(c2['fun'](res.x))
        assert res.success
