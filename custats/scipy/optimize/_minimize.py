...
from .cuslsqp.slsqp import _minimize_slsqp
...
def minimize(fun, x0, args=(), method=None, jac=None, hess=None,
    ...
    **Custom minimizers**
    ...
    """
    x0 = cp.array(x0)
    if cp.typename(x0.dtype.kind) == 'integer':
    ...
    if (meth in ('nelder-mead', 'cg', 'bfgs', 'newton-cg', 'dogleg',
                 'trust-ncg') and (bounds is not None or cp.any(cp.array(constraints)))):
        warn('Method %s cannot handle constraints nor bounds.' % method,
             RuntimeWarning)
    if meth in ('l-bfgs-b', 'tnc', 'powell') and cp.any(cp.array(constraints)):
